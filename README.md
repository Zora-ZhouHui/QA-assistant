...
这套结构需要的工程能力并不只是"调一下 SDK"：流式分片的 tool_calls 拼装、多轮工具循环的轮次控制、检索结果的统一编号与引用对齐、工具失败时的降级、长对话的上下文管理——都会在下面的[核心设计拆解](#核心设计拆解)中展开。
...
| `LLM_MODEL` | `deepseek-chat` | 模型名（如智谱则配 `glm-4.7-flash` + 对应 base_url） |
...
| `SEARCH_MAX_ROUNDS` | `2` | 单轮问答最多工具调用轮数（防失控、控延迟） |
...
每条记录包含 `question / answer / source_url / section` 等字段（见 [faq_loader.py](app/services/faq_loader.py)）。**只有 question 参与向量化**，answer 存在 metadata 中、检索命中后再拼回正文——这比把整篇 Q&A 向量化更符合"问题对问题"的语义匹配。
...
核心循环在 [app/agent/loop.py](app/agent/loop.py)：每次循环都是一次模型调用——要么请求工具，要么直接生成回答。工具结果以 OpenAI 协议要求的 `role: "tool"` 消息回灌后进入下一轮。当工具调用轮数达到 `SEARCH_MAX_ROUNDS`，下一轮请求**直接收回全部工具 schema**，模型只能基于已有信息收尾作答，从机制上杜绝"无限搜索"。

### 2. 流式 tool_calls 分片拼装

流式协议下，一次工具调用的 `id / 函数名 / arguments JSON` 是按分片增量到达的（arguments 甚至可能半个字符一切）。[app/core/llm.py](app/core/llm.py) 按 `index` 累加 arguments 拼装出完整调用结构，对上层屏蔽分片细节；并以"是否真的收到过分片"而非 `finish_reason` 判定工具调用——不同厂商兼容层的 finish_reason 并不总是规范。

### 3. 双 Collection 合并检索

Chroma 中维护两张表（[vectorstore.py](app/core/vectorstore.py)）：`docs` 存长文档切片（一篇多片），`faq` 存一问一答。两表都用 cosine 空间，分数天然可比：检索时各取 `top_k×2` 候选，**跨表按相似度全局排序**后返回 top_k，结果带 `collection` 字段供前端区分渲染。
...
近轮原文估算 token 超过 `MEMORY_WINDOW_TOKENS` 时，后台任务（不阻塞 SSE）把最老的一批消息交给 LLM 压成 ≤300 字摘要并从工作窗口移除，直到回落到 `MEMORY_TARGET_TOKENS` 以下。**越老越压缩、越新越完整**；全量原文永远保留在归档中，历史查询不丢内容。session_id 做白名单校验（8～128 位字母数字），同时防住路径穿越。
...
- **无检索重排序（rerank）**：当前直接取向量相似度 top_k，长文档/强干扰场景可加 cross-encoder rerank 提升精度。
- **历史会话不回放资料卡片**：归档只存问答文本，切回历史会话时引用角标会被移除（当前轮次内角标与卡片完整联动）。
- **无自动化测试**：核心链路（切片、双表合并、工具循环、压缩）目前靠手工验证与执行日志观测，缺少 pytest 覆盖。
- 模型需支持 OpenAI 标准 function calling；极小众兼容层可能存在流式协议差异（代码已对 finish_reason 不规范做兜底）。
